"""
Linear Kalman Filter Implementation — SOLUTION

State: x = [x, y, vx, vy]^T  (position and velocity in 2D)

PREDICT runs on every odometry callback (~30 Hz)
UPDATE runs on every GPS callback (~1 Hz)
"""
...
class LinearKalmanFilter:
    ...
    Fuses fast odometry (predict) with slow GPS (update).
    ...
    def predict(self):
        """
        Prediction step — called on every odom callback.
    ...
    def update(self, z_x: float, z_y: float):
        """
        Update step — called on every GPS callback.
